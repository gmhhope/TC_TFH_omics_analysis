...
Workdir <- "/Volumes/LM_MG_drive_one/Metabolomics_Transcriptome_integration_06042020/Pathway_wise_analysis/TC_Tfh_metabolomics/GSEA_output_july12/mitophagy_related_Tn_TCvsB6.GseaPreranked.1598783337239"
setwd(Workdir)

library(Rtoolbox)
## Script by Thomas Kuilman
## path argument: Yespath to output folder of analysis (e.g. PATH/my_analysis.GseaPreranked.1470948568349)
## gene.set argument: name of the gene set (e.g. V$AP1_Q2).
## It is used in a grep command, so multiple matching is possible.
## Also, R regular expressions can be handled, e.g. "IL2[0-9]$"
## Leading "V$" from gene set names are stripped to allow using the grep command.
...
path <- Workdir
# #gene.set <- "GO_LIPOPOLYSACCHARIDE_MEDIATED_SIGNALING_PATHWAY"
class.name <- "TC versus B6 (Tn)"
# 
# replotGSEA(path = path, gene.set = gene.set, class.name = class.name)

gene.set.vector <- c("REACTOME_PINK_PARKIN_MEDIATED_MITOPHAGY", "REACTOME_MITOPHAGY")
for (i in (1:length(gene.set.vector))) {
  replotGSEA(path = path, gene.set = gene.set.vector[i], class.name = class.name)
}