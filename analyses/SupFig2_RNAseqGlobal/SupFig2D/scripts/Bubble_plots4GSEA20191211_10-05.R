setwd("/Volumes/LM_MG_drive_one/Morel_meta_analyses_transcriptomics/GSEA_TfhvsTn_meta_TC_B6_noTC2DG/GSEA_Hallmark_meta_TfhvsTn/")

file_name = "meta_df4bubble_padj0.25.csv"
df_filt <- read.csv(file_name, header = TRUE)
colnames(df_filt)
df_filt$log10FDR<- -log(df_filt$FDR.q.val,10)
df_filt[df_filt$log10FDR==Inf,"log10FDR"] = 5
df_filt$group <- factor(df_filt$group, levels = c("TfhvsTn_B6","TfhvsTn_TC"))
...
library(ggplot2)
    ...
    pdf(paste("ggplot_bubble_",str_extract(file_name, '^[^.]+'),".pdf", sep = "") , width = 5.4, height = 6)
