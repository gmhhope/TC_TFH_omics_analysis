df = read.csv("./input/hallmark_FC1.5_without_direction.csv")
df$log10padj = -log(df$FDR.q.value,10)
df$group = "overlapped_TCvsB6_Tfh_Tn"
df$size = as.numeric(df$X..Genes.in.Overlap..k.)
df$Gene.Set.Name = factor(df$Gene.Set.Name, levels=unique(df$Gene.Set.Name[order(df$log10padj,decreasing = FALSE)]),ordered=TRUE)
...
pdf("output/dotplot_GSEAoverlap_091220.pdf", height = 3, width = 5.5)
print(S1)
dev.off()